...
func GetFingerList(file string) ([]model.Fingers, []model.Fingers, error) {
	var (
		finger  model.PackFingers
		keyword []model.Fingers
		favicon []model.Fingers
		data    []byte
	)
	if file == "" {
		data = FingerByte
	...
	multiErr := json.Unmarshal(data, &finger)
	if multiErr != nil {
		return nil, nil, multiErr
	}
	for _, v := range finger.Fingerprint {
		if v.Method == "keyword" {
			keyword = append(keyword, v)
		} else {
			favicon = append(favicon, v)
		}
	}
	return keyword, favicon, nil
}
