...
type Keywords struct {
	Finger []model.Fingers
}

func (k *Keywords) Match(header string, title string, body string) string {
	for _, kSub := range k.Finger {
		var b = false
		location := kSub.Location
		switch {
		case location == "body":
			location = body
		...
		for _, v := range kSub.Keyword {
			if strings.Contains(location, v) == true {
				b = true
			} else {
				b = false
				break
