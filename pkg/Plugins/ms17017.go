...
func MS17010(info *options.HostInfo) error {
...
func MS17010Scan(info *options.HostInfo) error {
	ip := info.Host
	...
	if reply[9] == 0x05 && reply[10] == 0x02 && reply[11] == 0x00 && reply[12] == 0xc0 {
		...
		if reply[34] == 0x51 {
			...
			res := common.VulInfo{"plugin", info.TaskID, map[string]interface{}{"type": "ms17010", "result": result}}
			common.LogSuccess(&res)
			global.Log.Warn("plugin", zap.String("ms-17-010", "ms-17-010"), zap.String("host", info.Host),
				zap.String("result", result))
			model.PluginList = append(model.PluginList, model.Plugin{"ms-17-010", result, info.Host, info.Ports})
		}

	} else {
		result := fmt.Sprintf("[*] %s  (%s)", ip, os)
		res := common.VulInfo{"plugin", info.TaskID, map[string]interface{}{"type": "ms17010", "result": result, "host": info.Host, "port": 445}}
		common.LogSuccess(&res)
		global.Log.Warn("plugin", zap.String("ms-17-010", "ms-17-010"), zap.String("host", info.Host),
			zap.String("result", result))
		model.PluginList = append(model.PluginList, model.Plugin{"ms-17-010", result, info.Host, info.Ports})
