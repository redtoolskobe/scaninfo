...
func NetBIOS(info *options.HostInfo) error {
	...
	if len(nbname.group) > 0 || len(nbname.unique) > 0 {
		res := common.VulInfo{"plugin", info.TaskID, map[string]interface{}{"type": "NetBIOS", "result": msg, "host": info.Host}}
		common.LogSuccess(&res)
		global.Log.Warn("plugin", zap.String("NetBIOS", "NetBIOS"), zap.String("host", info.Host),
			zap.String("result", msg))
		model.PluginList = append(model.PluginList, model.Plugin{"NetBIOS", msg, info.Host, info.Ports})
	}
	return err
}

func NetBIOS1(info *options.HostInfo) (nbname NbnsName, err error) {
...
func GetNbnsname(info *options.HostInfo) (nbname NbnsName, err error) {
